...
const TicketListScreen = ({ route, navigation }) => {
  ...
  const filters = [
    {
      ...
      label: "Todos",
      icon: (
        <MaterialIcons name="list-alt" size={16} color={colors.textTertiary} />
      ),
    },
    {
      value: "open",
      label: "Abertos",
      icon: <MaterialIcons name="circle" size={16} color={colors.success} />,
    },
    {
      value: "in_progress",
      label: "Em Andamento",
      icon: <MaterialIcons name="circle" size={16} color={colors.warning} />,
    },
    {
      value: "resolved",
      label: "Resolvidos",
      icon: <MaterialIcons name="circle" size={16} color={colors.primary} />,
    },
    {
      value: "closed",
      label: "Fechados",
      icon: <MaterialIcons name="circle" size={16} color="#9E9E9E" />,
    },
  ];
  ...
  const getStatusInfo = (status) => {
    const statusMap = {
      ...
      in_progress: {
        label: t("ticketListScreen.status.in_progress"),
        color: colors.warning,
      },
      resolved: {
        label: t("ticketListScreen.status.resolved"),
  ...
  const formatDate = (dateString) => {
    ...
    if (diffDays < 7)
      return t("ticketListScreen.date.daysAgo", { count: diffDays });
  ...
  const renderTicket = ({ item }) => {
    ...
    const unreadCount = item.messages
      ? item.messages.filter((m) => !m.read && m.sender === "support").length
      : 0;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>
          {t("ticketListScreen.header.title")}
        </Text>
        <Text style={styles.headerSubtitle}>
          {t("ticketListScreen.header.subtitle", {
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>
            {t("ticketListScreen.loading.text")}
          </Text>
        </View>
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>
            {t("ticketListScreen.empty.title")}
          </Text>
          <Text style={styles.emptySubtext}>
            {selectedFilter === "all"
              ? t("ticketListScreen.empty.allSubtitle")
              : t("ticketListScreen.empty.filteredSubtitle", {
                  filter: filters
          ...
          {selectedFilter === "all" && (
            ...
            >
              <Text style={styles.createButtonText}>
                {t("ticketListScreen.actions.create")}
              </Text>
            </TouchableOpacity>
          )}
        </View>
